...
def get_options():
    import argparse
    description="""Generate basic golang code for state machine passed as input."""
    ...
    help = "The destination filename to generate the golang code into."
    parser.add_argument("--golang_output_filename", required=True, help=help)
...
class Test1(object):
    ...
    def run_test(self):
        ...
        #golang_filename = "test1x.go"

        
        options = get_options()
        input_filename = options.input_filename
        golang_filename = options.golang_output_filename
        ...
        with open(golang_filename, "w") as hFile:
            scprint = Printer(hFile)

            self.print_golang_code(input_filename, 
                                   parsed_model,
                                   state_list,
                                   transition_list,
                                   state_tree_root,
    ...
    def print_golang_code(self, 
        ...
        DISPATCH = "switch source := eventSource.(type) {"

        for source in sorted(sources):
            dispatch_string = """
    case *{source_type}: return sm.dispatch_{source_type}(source, ev);
"""
...
#
        ...
        for portname in sorted(sources):
            PORTS += """
    //%s_port PortBinding<Owner, %s, EventBase>
""" % (portname, portname.lower(),)

        ports_class = """
type Model_{machine_name} interface {{
{methods_string}
}};

type Ports_{machine_name} interface
{{
{PORTS}
}};

type PortsModel_{machine_name} interface
{{
    Model_{machine_name}
    Ports_{machine_name}
}};
...
type {machine_name} struct {{
    // {baseclassname}

    initialised bool
    state uint64
    instanceid string
    model Model_{machine_name}
}}


func (sm *{machine_name}) State() uint64 {{
        return sm.state
}}

func (sm *{machine_name}) SetState(value uint64) {{
        sm.state = value
}}

func (sm *{machine_name}) IsInitialised() bool {{
        return sm.initialised
}}

func (sm *{machine_name}) SetInitialised(value bool) {{
        sm.initialised = value
}}

func (sm *{machine_name}) Initialise() {{
        sm.initialise_statemachine();
}}

func (sm *{machine_name}) UnhandledEvent(source EventSource, ev Event) error {{
        fmt.Printf("UNHANDLED_EVENT: %#v %#v\\n", source, ev)
        return nil
}}

func (sm *{machine_name}) Dispatch(eventSource EventSource, ev Event) error {{
    {DISPATCH}
...
func (sm *{machine_name}) Send(source EventSource, ev Event) error {{
    return sm.Dispatch(source, ev)
}}


func (sm *{machine_name}) InstanceId() (string) {{ 
    return "sm.instanceid"; 
}}

func (sm *{machine_name}) Model() (model Model_{machine_name}) {{ 
    return sm.model
}}

func (sm *{machine_name}) SetModel(model Model_{machine_name}) {{ 
    sm.model = model
}}

func (sm *{machine_name}) ModelName() (string) {{ 
    return "{machine_name}"; 
}}

func (sm *{machine_name}) ModelNamespace() (string) {{ 
    return "{namespace}"; 
}}

func (sm *{machine_name}) ModelGuid() (string) {{ 
    return "{header.modelguid}"; 
}}

func (sm *{machine_name}) ModelFilename() (string) {{ 
    return "{header.modelfilename}"; 
}}

func (sm *{machine_name}) StatemachineVersion() (string) {{ 
    return "{header.statemachineversion}"; 
}}

func (sm *{machine_name}) Comment() (string) {{ 
...
"""
        ...
        for eventname, ev_transition_items in sorted(event_list_transitions.items()):
            print()
            print("func (sm *{machine_name}) process_event_{eventsource}_{eventname}(source * {eventsource}, ev *{eventname}) error {{"
                  .format(machine_name=machine_name,
                          eventname=eventname[1], 
                          eventsource=eventname[0]))
            fromstate_transitions = generate_fromstate_transitions(ev_transition_items)
            offset = "    "
            print("{offset}model := sm.model".format(**locals()))
                ...
                for (state, 
                    ...
                    if True:
                        ...
                        if transition.guard:
                            ...
                            print("%s%sif(model.%s) {" % (
                                offset, else_value, make_call(transition.guard)))
            ...
            print("{offset}{offset}sm.UnhandledEvent(source, ev);".format(**locals()))
            print("{offset}}}; break;".format(**locals()))
            print("{offset}}}".format(**locals()))
            print("{offset}return nil".format(**locals()))
        ...
        for source in sorted(sources):
            print()
            print("func (sm *{machine_name}) dispatch_{event_source}(source *{event_source}, event Event) error {{".format(
                machine_name=machine_name,
                event_source=source))
            print("%sif(!sm.IsInitialised()) {" % (offset,))
            print("%s%sreturn errors.New(\"Statemachine \" + sm.ModelName() + \", \" + sm.ModelGuid() + sm.InstanceId() + \" is not initialised\");" % (offset, offset))
            print("%s}" % (offset,))
            offset = "    "
            print("%sswitch ev := event.(type) {" % (offset,))
            for eventname, ev_transition_items in sorted(event_list_transitions.items()):
                if eventname[0] != source:
                    continue
                print("{offset}case *{event_name}: return sm.process_event_{event_source}_{event_name}(source, ev)".format(
                    offset=offset,
                    event_source=source,
                    event_name=eventname[1]))
                
                pass
            print("%sdefault:" % (offset,))
            print("%s%sreturn errors.New(\"Unsupported event\");" % (offset, 
                                                                          offset,))
            print("%s}" % (offset,))
        ...
        print("func (sm *{machine_name}) initialise_statemachine() error {{".format(
                machine_name=machine_name))
        print("%sif(sm.IsInitialised()) {" % (offset,))
        print("%s%sreturn errors.New(\"Statemachine \" + sm.ModelName() + \", \" + sm.ModelGuid() + sm.InstanceId() + \" is not initialised\");" % (offset, offset))
        print("%s}" % (offset,))
        ifstatement = "if"
        offset = "    "
        print("%ssm.SetState(%s);" % (offset, "TOPSTATE",))
        for fstate in get_start_state_descendent_node_list_from(state_tree_root):
            if fstate.state().entry:
                print("%ssm.model.%s;" % (offset, fstate.state().entry,), "// initialise by walking default states")
            print("%ssm.SetState(%s);" % (offset, fstate.state().name,))
        print("%ssm.SetInitialised(true);" % (offset,))
        print("%sreturn nil;" % (offset,))
        print("}")

        print()

        print("// gen complete -- class ", machine_name)
        print()
